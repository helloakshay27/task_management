...
const NewMilestoneDateEditor = ({ value, onChange, onEnterPress, placeholder, validator, min }) => {
  const handleKeyDown = (event) => {
    if (event.key === 'Enter' && onEnterPress) {
      event.preventDefault();
      onEnterPress();
  ...
  return (
    <>
      <div className="action-icons flex justify-around items-center">
        <button onClick={() => navigate(`${row.original.actualId}`)} title="View Details">
...
const globalStatusOptions = ['open', 'in_progress', 'on_hold', 'completed', 'overdue'];
...
const MilestoneList = ({ searchQuery, selectedColumns }) => {
  ...
  const { loading: statusChangeLoading } = useSelector((state) => state.updateMilestone);
  ...
  const [newMilestoneStatus, setNewMilestoneStatus] = useState(globalStatusOptions[0]);
  ...
  const transformedData = useMemo(() => {
    ...
    if (!Array.isArray(milestones)) {
      if (milestones?.data && Array.isArray(milestones.data)) {
        return milestones.data.map((milestone, index) => transformMilestone(milestone, index));
      }
    ...
    return milestones.map((milestone, index) => transformMilestone(milestone, index));
    ...
    function transformMilestone(milestone, index) {
      try {
        ...
        console.error(`Error transforming milestone ${index}:`, error, 'Milestone:', milestone);
        return {
          id: `M-error-${index}`,
  ...
  const handleStatusChange = useCallback(
    async ({ id: rowId, name, payload: newValue }) => {
      ...
      try {
        ...
        console.error(`Failed to update milestone ${name} for ID ${actualMilestoneId}:`, err);
      }
    },
    [dispatch, id]
  );
      ...
      <span onDoubleClick={handleDoubleClick}>
        {edit ? (
          ...
          <span
            ...
            onClick={() => navigate(`${row.original.actualId}/tasks`)}
  ...
  const handleSaveNewMilestone = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        error?.response?.data?.message || error?.message || 'Failed to save milestone.';
    ...
    () => [
      {
        ...
        cell: ({ getValue }) => (
          <span
            ...
            onClick={() => navigate(`${getValue().replace('M-', '')}`)}
          >
            {getValue()}
          </span>
        ),
    ...
    const newMilestoneFields = {
      ...
      status: (
        <td key="status" className="p-1 border-r-2 align-middle">
          <StatusBadge
            statusOptions={globalStatusOptions.map((s) => s.charAt(0).toUpperCase() + s.slice(1))}
            status={newMilestoneStatus.charAt(0).toUpperCase() + newMilestoneStatus.slice(1)}
            onStatusChange={(val) => setNewMilestoneStatus(val.toLowerCase())}
          />
        </td>
      ),
      owner: (
        <td key="owner" className="p-0 border-r-2 align-middle">
          <SelectBox
            ...
            onChange={(selectedValue) => setNewMilestoneOwner(selectedValue)}
            table={true}
  ...
  if (fetchMilestonesLoading || isSavingNewMilestone) {
    const loadingMessage = isSavingNewMilestone ? 'Saving Milestone...' : 'Loading Milestones...';
            ...
            <tbody>
              {data.length === 0 && !isAddingNewMilestone ? (
                <tr style={{ height: `${rowHeight}px` }}>
                  ...
                  >
                    No milestones found. {searchQuery ? 'Try adjusting search.' : ''}
                  </td>
                </tr>
              ) : (
