  ...
  return (
    ...
    >
      <p
        ...
        onClick={() => navigate(`${parentMilestoneId}/tasks/${task.id}`)}
      >
        <span className="text-blue-500">T-{task.id}</span>{' '}
        {task.title.replace(/@\[(.*?)\]\(\d+\)/g, '@$1').replace(/#\[(.*?)\]\(\d+\)/g, '#$1')}
      </p>
