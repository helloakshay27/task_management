...
const MilestoneDependencyTable = () => {
  ...
  const handleAddMilestone = async () => {
    if (
      !newMilestone.title ||
      !newMilestone.owner_id ||
      !newMilestone.start_date ||
      !newMilestone.end_date
    ) {
      toast.error('Please fill all required fields');
      return;
    }
                  ...
                  <>
                        ...
                        <button
                          onClick={(e) => {
                            ...
                            setOpenOwnerDropdown(
                              openOwnerDropdown === milestone.id ? null : milestone.id
                            );
                          }}
                          disabled={isSaving}
                        ...
                        {openOwnerDropdown === milestone.id && (
                          ...
                          >
                            {userOptions ? (
                              ...
                              <span className="w-full px-4 py-2 text-sm hover:bg-gray-100 text-left text-gray-800">
                                No users found
                              </span>
                            )}
                          </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-sm font-medium ${getDurationColor(calculateDuration(editData.start_date, editData.end_date))}`}
                        >
                  ...
                  <>
                    ...
                    >
                      <div className="flex items-center gap-2 text-sm">
                        <span
                          className={`w-2 h-2 rounded-full ${getStatusColor(milestone.status)}`}
                        ></span>
                        <span className="text-gray-800">
                          {mapStatusToDisplay(milestone.status)}
                        </span>
                      </div>
                    </td>
                    <td
                    ...
                    >
                      <span
                        className={`text-sm font-medium ${getDurationColor(milestone.duration)}`}
                      >
              ...
              <tr className="border-b border-gray-200 bg-blue-50">
                ...
                <td className="!px-6 py-4">
                  <div className="relative">
                    ...
                    >
                      <span className="text-gray-800">
                        {newMilestone.owner_name || 'Select owner'}
                      </span>
                      <ChevronDown className="w-4 h-4 text-gray-400" />
                    </button>
                    {openOwnerDropdown === 'new' && (
                      ...
                      >
                        {userOptions ? (
                          ...
                          <div className="w-full px-4 py-2 text-sm text-left text-gray-800">
                            No users found
                          </div>
                        )}
                      </div>
                ...
                <td className="px-6 py-4">
                  <span
                    className={`text-sm font-medium ${getDurationColor(calculateDuration(newMilestone.start_date, newMilestone.end_date))}`}
                  >
