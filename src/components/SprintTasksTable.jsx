...
const SprintTasksTable = ({ sprint }) => {
  ...
  const saveEditing = async () => {
    ...
    if (
      !editData.title ||
      !editData.responsible_person_id ||
      !editData.expected_start_date ||
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-200">
              ...
              <th className="text-left px-6 py-3 text-gray-700 font-medium text-sm">
                Responsible Person
              </th>
                ...
                {editingId === task.task_management.id ? (
                  <>
                    ...
                    <td className="px-6 py-4">
                      <div className="relative">
                        ...
                        >
                          <span
                            className={`w-2 h-2 rounded-full ${getStatusColor(editData.status)}`}
                          ></span>
                        ...
                        {openDropdown === task.task_management.id && (
                          <div
                            className="fixed bg-white border border-gray-200 rounded shadow-lg z-50 min-w-[150px]"
                      ...
                      <div className="relative">
                        ...
                        >
                          <span className="text-gray-800">
                            {editData?.responsible_person?.name}
                          </span>
                          <ChevronDown className="w-4 h-4 text-gray-400" />
                        </button>
                        {openOwnerDropdown === task.task_management.id && (
                          <div
                            className="fixed bg-white border border-gray-200 rounded shadow-lg z-50 flex flex-col max-h-[150px] overflow-y-auto"
                            style={{
                              top: dropdownPos.top,
                    ...
                    <td className="px-6 py-4">
                      <input
                        ...
                        onChange={(e) =>
                          setEditData({ ...editData, expected_start_date: e.target.value })
                        }
                        onKeyPress={handleKeyPress}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-sm font-medium ${getDurationColor(calculateDuration(editData.expected_start_date, editData.target_date))}`}
                        >
                  ...
                  <>
                    ...
                    <td
                      ...
                      onClick={() => startEditing(task.task_management)}
                    >
                    ...
                    >
                      <div className="flex items-center gap-2 text-sm">
                        <span
                          className={`w-2 h-2 rounded-full ${getStatusColor(task.task_management.status)}`}
                        ></span>
                        <span className="text-gray-800">
                          {mapStatusToDisplay(task.task_management.status)}
                        </span>
                      </div>
                    </td>
                    <td
                      className="!px-6 py-4 text-gray-800 text-sm cursor-pointer"
                      onClick={() => startEditing(task.task_management)}
                    >
                    ...
                    >
                      <span
                        className={`text-sm font-medium ${getDurationColor(task.task_management.duration)}`}
                      >
                        {calculateDuration(
                          task.task_management.expected_start_date,
                          task.task_management.target_date
                        )}
                      </span>
                    </td>
                    <td
              ...
              <tr className="border-b border-gray-200 bg-blue-50">
                ...
                <td className="!px-6 py-4">
                  <div className="relative">
                    ...
                    >
                      <span className="text-gray-800">
                        {newTask.responsible_person_name || 'Select responsible person'}
                      </span>
                      <ChevronDown className="w-4 h-4 text-gray-400" />
                    </button>
                ...
                <td className="px-6 py-4">
                  <span
                    className={`text-sm font-medium ${getDurationColor(calculateDuration(newTask.start_date, newTask.end_date))}`}
                  >
