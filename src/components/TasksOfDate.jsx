import { ChevronUp, ChevronDown, CalendarCheck2, Timer, FlagTriangleRight } from 'lucide-react';
  ...
  return (
    <div className="bg-white border-gray-300 relative">
      ...
      <div className="flex items-center justify-center absolute -top-[23px] left-[50%] translate-x-[-50%] z-10">
        <button type="button" onClick={() => onScroll('up')} className="p-1 rounded">
      ...
      <div className="flex items-center justify-center absolute -bottom-[25px] left-[50%] translate-x-[-50%] z-10">
        <button type="button" onClick={() => onScroll('down')} className="p-1 rounded">
...
const TasksOfDate = ({ selectedDate, onClose, tasks, userAvailability, selectedUser, shift }) => {
  ...
  const weekDates = useMemo(() => {
    ...
    const sorted = [...userAvailability].sort((a, b) => new Date(a.date) - new Date(b.date));
    ...
    const visible = sorted.slice(dateStartIndex, dateStartIndex + visibleDaysCount);
  ...
  const handleDrop = async (task, dateInfo) => {
    ...
    const formatedSelectedDate = `${selectedDate.year}-${(selectedDate.month + 1)
      .toString()
  ...
  const selectedDayAvailability = userAvailability?.find((u) => u.date === selectedDateString);
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="bg-white mt-3">
        ...
        <div className="bg-red-600 text-white px-4 py-3 flex items-center justify-between">
          ...
          <div className="font-semibold">Total Allocated Task Hours: {totalTaskHours}</div>
        </div>
          ...
          <div className="col-span-2 relative">
            {hasPreviousTasks && (
              <div className="flex justify-center mb-1 absolute -top-[18px] left-[50%] translate-x-[-50%] z-10">
                <button className="rounded" type="button" onClick={() => handleTaskScroll('up')}>
              ...
              >
                {currentTasks.length > 0 ? (
                  currentTasks.map((task) => (
                    <TaskCard key={task.id} task={task} selectedDate={selectedDate} />
                  ))
                ) : (
                  <div className="text-center text-gray-500 text-sm">No tasks available</div>
                )}
              </div>
            </div>
            ...
            {hasMoreTasks && (
              <div className="flex justify-center mt-1 absolute -bottom-[16px] left-[50%] translate-x-[-50%] z-10">
                <button className="rounded" type="button" onClick={() => handleTaskScroll('down')}>
