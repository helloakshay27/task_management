...
const TYPE_OPTIONS = [
  ...
  {
    key: 'Kanban',
    icon: <ChartNoAxesColumn size={18} className="rotate-180 text-[#C72030]" />,
    label: 'Kanban',
  },
...
const SPRINT_TYPE_OPTIONS = [
  ...
  {
    key: 'Sprint-Gantt',
    icon: <ChartNoAxesGantt size={20} className="text-[#C72030]" />,
    label: 'Gantt',
  },
];
...
const TaskActions = ({
  ...
  const [selectedStatus, setSelectedStatus] = useState(STATUS_OPTIONS_MAP[addType]?.[0] || 'All');
  ...
  useEffect(() => {
    if (addType === 'Sprint-Gantt') {
      ...
      if (savedSprintStatus) {
        ...
        const capitalizedStatus = statusLabel
          .split(' ')
    ...
    try {
      if (addType === 'Task') {
        ...
        const response = await axios.get(
          `${baseURL}/task_managements/export_tasks.json?${queryParams.toString()}`,
          {
            responseType: 'blob',
            headers: {
      ...
      <div className="relative ml-2">
        ...
        {isTypeOpen && (
          <ul className="absolute left-0 mt-2 w-[150px] bg-white border border-gray-300 shadow-xl rounded-md z-10">
            {TYPE_OPTIONS.filter(({ key }) => addType === 'Milestone' || key !== 'Gantt').map(
              ({ key, icon, label }) => (
                <li key={key}>
                  <button
    ...
    <>
      <div className="flex items-center justify-between mx-6 mt-4 mb-3 text-sm">
            ...
            selectedType === 'List' && (
              <>
                <input
                  ...
                  type="text"
                  className="border border-gray-300 ps-10 pe-2 py-2 w-[350px] focus:outline-none"
        ...
        <div className="flex items-center gap-3 divide-x divide-gray-400">
          ...
          {addType !== 'Issues' &&
            addType !== 'Opportunity' &&
            !['Milestone', 'Project', 'Task', 'active_projects', 'templates', 'archived'].includes(
              addType
            ) &&
      ...
      {isFilterModalOpen && (
        <TaskFilter isModalOpen={isFilterModalOpen} setIsModalOpen={setIsFilterModalOpen} />
      )}
      ...
      {isAddIssueModalOpen && (
        <AddIssueModal isModalOpen={isAddIssueModalOpen} setIsModalOpen={setIsAddIssueModalOpen} />
      )}

      {isIssueFilter && (
        <IssueFilter isModalOpen={isIssueFilter} setIsModalOpen={setIsIssueFilter} />
      )}
