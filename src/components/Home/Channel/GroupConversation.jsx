...
const GroupConversation = () => {
  ...
  const handleRemoveMember = async (userId) => {
    try {
      await dispatch(removeUserFromGroup({ baseUrl, token, userId })).unwrap();
      setShowGroupDetails(false);
      fetchData();
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto">
        {activeTab === 'chat' && id && (
          <Chats messages={messages} onReply={handleReply} bottomRef={bottomRef} />
        )}
      ...
      {activeTab === 'chat' && (
        <div className={`w-[calc(100vw-30rem)] mx-auto px-6 py-4 flex items-center space-x-2`}>
            ...
            <div className="bg-white rounded-2xl shadow-md p-3 flex flex-col">
              ...
              {replyingTo && (
                <div className="mb-2 bg-gray-50 border-l-4 border-[#C72030] rounded p-2 flex items-start justify-between">
                  ...
                  <button onClick={cancelReply} className="ml-2 text-gray-400 hover:text-gray-600">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              )}
