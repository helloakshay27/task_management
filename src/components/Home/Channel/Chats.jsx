...
const Chats = ({ messages, onReply, bottomRef }) => {
  ...
  const handleSendForward = async () => {
    ...
    try {
      ...
      toast.success(
        `Message forwarded to ${selectedForwardTargets.length} ${selectedForwardTargets.length === 1 ? 'chat' : 'chats'}`
      );
  ...
  const renderAttachments = (attachments) => {
    ...
    const imageFiles = attachments.filter((a) => a?.content_type?.startsWith('image/'));
    const otherFiles = attachments.filter((a) => !a?.content_type?.startsWith('image/'));
        ...
        {imageFiles.length > 0 && (
          ...
          >
            {imageFiles.slice(0, 4).map((file, i) => {
              ...
              const extraCount = imageFiles.length > 4 ? imageFiles.length - 4 : 0;
  ...
  const renderReplyPreview = (parentMessage, isMe) => {
    ...
    return (
      <div
        className={`mb-2 border-l-4 ${isMe ? 'border-gray-400 bg-gray-50' : 'border-[#C72030] bg-red-50'} rounded p-2`}
        onClick={() => scrollToMessage(parentMessage.id)}
      >
        <div className="text-xs font-semibold text-gray-700 mb-1">{parentMessage.user_name}</div>
        <div className="text-xs text-gray-600 truncate">{parentMessage.body || 'Attachment'}</div>
      </div>
    );
  };
      ...
      <div className="flex-1 w-full overflow-y-auto max-h-[calc(100vh-160px)] px-6 py-4">
        {[...messages].reverse().map((message, index) => {
          ...
          return (
            <div
              key={index}
              className={`mb-6 flex flex-col transition-colors duration-300 ${
                isMe ? 'items-end' : 'items-start'
              }`}
              onMouseEnter={() => handleMouseEnter(index)}
              onMouseLeave={handleMouseLeave}
            >
              <div className={`text-xs text-gray-500 mb-2 ${isMe ? 'mr-12' : 'ml-[60px]'}`}>
              ...
              <ContextMenu>
                <ContextMenuTrigger asChild>
                  <div className="flex items-start space-x-3 cursor-pointer relative">
                    <div
                      className={`absolute top-1/2 -translate-y-1/2 flex gap-1 z-10 transition-all duration-300 ease-out ${
                        isMe ? 'left-[-65px]' : 'right-[-75px]'
                      } ${
                        shouldShowActions
                    ...
                    >
                      ...
                      {message.body && (
                        <div
                          className={`${
                            message.attachments?.length > 0 || parentMessage ? 'mt-2' : ''
                          }`}
                        >
            ...
            <div className="space-y-4">
                          ...
                          return (
                            <div
                              ...
                              className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors ${
                                isSelected ? 'bg-[#C72030] text-white' : 'hover:bg-gray-100'
                              }`}
                            >
                              <div
              ...
              {selectedForwardTargets.length > 0 && (
                <div className="text-sm text-gray-600">
                  ...
                  {selectedForwardTargets.length === 1 ? 'chat' : 'chats'} selected
                </div>
              )}
            </div>
        ...
        <Dialog open={!!selectedMessage} onOpenChange={() => setSelectedMessage(null)}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Do you want to create a task for this message?</DialogDescription>
            </DialogHeader>
            <div className="p-3 bg-gray-50 rounded-md text-sm text-gray-700">
              {selectedMessage?.body.slice(0, 100) +
                (selectedMessage?.body.length > 100 ? '...' : '')}
            </div>
