  ...
  const renderSinglePinned = () => {
    ...
    return (
      <div className="flex items-center gap-3 py-2 px-4 bg-[#F2EEE9] border-b">
        ...
        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => onMessageClick?.(message)}>
          <div className="text-xs font-semibold text-[#C72030] mb-0.5">{message.user_name}</div>
          <div className="text-sm text-gray-700 truncate">{truncateText(message.body)}</div>
        </div>
        <Button
          variant="ghost"
          size="sm"
