...
const TaskFilter = ({ isModalOpen, setIsModalOpen }) => {
  ...
  const [selectedResponsible, setSelectedResponsible] = useState(
    initialFilters.selectedResponsible
  );
  ...
  const [selectedWorkflowStatus, setSelectedWorkflowStatus] = useState(
    initialFilters.selectedWorkflowStatus || []
  );
  ...
  const [workflowStatusSearch, setWorkflowStatusSearch] = useState(
    initialFilters.workflowStatusSearch || ''
  );
  const [ResponsiblePersonSearch, setResponsiblePersonSearch] = useState(
    initialFilters.ResponsiblePersonSearch
  );
  ...
  const { tasks: filteredTasks } = useSelector((state) => state.filterTask);

  const { fetchUsers: users, loading, error } = useSelector((state) => state.fetchUsers);

  const { fetchStatus: workflowStatuses } = useSelector((state) => state.fetchStatus);
    ...
    const fetchProjects = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data.project_managements)) {
          setProjectOptions(
            response.data.project_managements.map((project) => ({
  ...
  const handleApplyFilter = (overrideFilters) => {
    ...
    try {
      const newFilter = {
        ...
        'q[project_status_id_in][]':
          selectedWorkflowStatus?.length > 0 ? selectedWorkflowStatus : [],
