...
export const DurationPicker = ({
  ...
  if (!Array.isArray(shift) && shift?.shift) {
    ...
    if (!startTime || !endTime) {
      ...
      if (
        shift.user_shift &&
        (shift.user_shift.break_start_hour != null || shift.user_shift.break_end_hour != null)
      ) {
  ...
  return (
    <div className={`relative ${className}`} ref={pickerRef}>
      ...
      <button
        ...
        disabled={disabled}
        type="button"
        className={`w-full px-4 py-[7px] bg-white border border-gray-300 shadow-sm focus:outline-none transition-colors flex items-center justify-between ${
          ...
          {taskType === 'flexible' ? (
            !endDate && !startDate ? (
              <div className="text-sm text-gray-500 bg-red-100 px-3 py-2 flex items-center gap-2">
                <Info size={16} /> Please select both start date and end date to enable flexible
                work hours.
              </div>
            ) : (
                      ...
                      {daysList.map((d, i) => (
                        <TableHead
                          ...
                          className={`min-w-[120px] text-center ${
                            !d.isWorking ? 'bg-gray-100 text-gray-500' : 'bg-white'
                          }`}
                        >
                          {d.formatted}
                        </TableHead>
                      ...
                      {daysList.map((d, idx) => (
                        ...
                        >
                          <input
                            ...
                            className={`w-16 border rounded-md text-center py-1 text-sm ${
                              !d.isWorking ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : ''
                            }`}
                          />
                          <div className="text-xs text-gray-500 mt-1">
                            {hoursPerDay > 0
                              ? ((parseHours(dailyHours[idx]) / hoursPerDay) * 100).toFixed(2)
                              : '0.00'}
                            %
                          </div>
                        </TableCell>
                ...
                <TableBody>
                  <TableRow>
                    ...
                    <TableCell className="text-right sticky right-0 bg-white border-l">
                      {totalWorkingHours > 0 ? `${formatTotalHours(totalWorkingHours)} hrs` : '--'}
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
