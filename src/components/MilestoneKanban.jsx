...
const MilestoneKanban = () => {
  ...
  const handleMilestoneStatusChange = useCallback(
    ({ id, status }) => {
      setMilestones((prev) => prev.map((mil) => (mil.id === id ? { ...mil, status } : mil)));
      handleStatusChange({
        id: `M-${id}`,
        payload: status,
      });
      ...
      >
        {cardsTitle.map((card) => {
          ...
          const filteredMilestone =
            milestones && milestones.filter((milestone) => milestone.status === cardStatus);

          // Get all tasks that belong in this status board, regardless of parent milestone status
          const tasksInThisStatus =
          ...
          return (
            ...
            >
                        ...
                        <div className="ml-5 mt-2 space-y-2">
                          {visibleTasks.map((task) => (
                            <div
                              ...
                              className="relative"
                            >
                              <MilestoneTaskCard task={task} parentMilestoneId={milestone.id} />
                            </div>
                          ))}
                        </div>
                      )}
              ...
              {tasksInThisStatus
                ...
                .map((task) => (
                  <div key={`task-${task.id}`} id={`task-${task.id}`} className="relative mt-2">
                    <MilestoneTaskCard task={task} parentMilestoneId={task.parentMilestone.id} />
                  </div>
                ))}
