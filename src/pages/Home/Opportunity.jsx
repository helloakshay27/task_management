...
const Opportunity = () => {
  ...
  const [columnOrder, setColumnOrder] = useState(() => {
    ...
    return savedOrder
      ...
      : ['id', 'title', 'project', 'task', 'status', 'created_by', 'created_at', 'action_taken'];
  });
  ...
  const allColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        size: 120,
        cell: ({ row }) =>
          row.original.task_created && row.original.project_management_id
            ? 'Converted to Project'
            : row.original.project_created && row.original.task_management_id
              ? 'Converted to Task'
              : row.original.task_created && row.original.milestone_id
                      ...
                      table.getRowModel().rows.map((row) => (
                        ...
                        >
                          {row.getVisibleCells().map((cell) => (
                            <td key={cell.id} className="border p-1 align-middle text-left">
