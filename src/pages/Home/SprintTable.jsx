...
const SprintTable = (setIsSidebarOpen) => {
  ...
  const { fetchSpirints: newSpirints, loading: sprintsLoading } = useSelector(
    (state) => state.fetchSpirints
  );

  const { fetchUsers: users, loading: usersLoading } = useSelector((state) => state.fetchUsers);

  const { loading: createSprintLoading } = useSelector((state) => state.postSprint);
  ...
  const allColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        size: 150,
        cell: ({ row }) => (
          <CountdownTimer startDate={row.original.start_date} targetDate={row.original.end_date} />
        ),
      },
      {
        id: 'priority',
