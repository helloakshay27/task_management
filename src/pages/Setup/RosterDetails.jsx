      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {Array.isArray(value) ? (
            value.length > 0 ? (
              value.map((item, idx) => (
                <span key={idx} className="bg-red-600 text-white px-3 py-1 rounded-full text-xs">
                  {item}
                </span>
              ))
            ) : (
            ...
            <span className="bg-red-600 text-white px-3 py-1 rounded-full text-sm">{value}</span>
          )}
        </div>
      </div>
    );
  ...
  const getWeekSelectionDisplay = () => {
    ...
    if (rosterTemplate.roaster_type === 'Recurring') {
      ...
      if (Array.isArray(rosterTemplate.no_of_days)) {
        return rosterTemplate.no_of_days.map(
          (weekNum) =>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor()} ${
                    rosterTemplate.active ? 'bg-green-50' : 'bg-red-50'
                  }`}
                >
                  {getStatus()}
                </span>
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <InfoField label="Frequency" value={getWeekSelectionDisplay()} variant="badge" />
          </div>
        </div>
