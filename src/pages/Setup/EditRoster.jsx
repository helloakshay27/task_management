...
const EditRoster = () => {
  ...
  useEffect(() => {
    const fetchRosterData = async () => {
      ...
      try {
        ...
        if (
          data.roaster_type === 'Recurring' &&
          data.no_of_days &&
          Array.isArray(data.no_of_days) &&
          data.no_of_days.length > 0
        ) {
        ...
        const departments =
          data.departments && Array.isArray(data.departments)
            ? data.departments.map((dept) => dept.id)
            : data.department_id
              ? Array.isArray(data.department_id)
                ? data.department_id.map(Number)
    ...
    try {
      ...
      setFilteredUsers(
        users.map((user) => ({
          id: user.id,
          name:
            user.name || user.full_name || `${user.firstname || ''} ${user.lastname || ''}`.trim(),
          email: user.email,
  ...
  const validateForm = () => {
    ...
    const newErrors = {
      ...
      selectedEmployees: formData.departments.length > 0 && formData.selectedEmployees.length === 0,
    };
          ...
          <div>
            ...
            {formData.dayType === 'Weekdays' && (
              <div className="space-y-3 p-4 bg-[#f6f4ee] border border-[#D5DbDB]">
                ...
                <div className="flex flex-wrap gap-2">
                  {['1st Week', '2nd Week', '3rd Week', '4th Week', '5th Week', 'All'].map(
                    (week) => (
                      <label
                        key={week}
                        className={`
            ...
            {formData.dayType === 'Weekends' && (
              <div className="space-y-3 p-4 bg-[#f6f4ee] border border-[#D5DbDB]">
                ...
                <div className="text-xs text-[#1a1a1a] opacity-70 mt-2">Days: Sat, Sun</div>
              </div>
            )}
                        ...
                        onClick={() => {
                          ...
                          setFormData((prev) => ({
                            ...
                            selectedDays: hasAllDays
                              ...
                              : [
                                  ...prev.selectedDays,
                                  ...allDaysForWeek.filter((d) => !prev.selectedDays.includes(d)),
                                ],
                          }));
                        }}
                        disabled={isSubmitting}
        ...
        <Section title="Shift & Employees" icon={<Clock className="w-4 h-4" />}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="relative">
              ...
              {errors.shift && <p className="text-red-500 text-sm mt-1">Please select a shift</p>}
            </div>
            {formData.departments.length > 0 && (
              <div className="relative">
                ...
                >
                  <InputLabel shrink>
                    List Of Selected Employees <span className="text-red-500">*</span>
                  </InputLabel>
                  <MuiSelect
                    multiple
                    value={formData.selectedEmployees}
                    onChange={(e) => handleInputChange('selectedEmployees', e.target.value)}
                    ...
                    displayEmpty
                    disabled={
                      loadingFilteredUsers || isSubmitting || formData.departments.length === 0
                    }
        ...
        {formData.departments.length === 0 && (
          <Section title="Employees" icon={<Users className="w-4 h-4" />}>
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 text-lg font-medium mb-2">Select Departments First</p>
              <p className="text-gray-400 text-sm">
                Please select at least one department to view and select employees
              </p>
            </div>
          </Section>
        )}
              ...
              <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="flex items-center gap-2 text-sm text-blue-800">
                  ...
                  {period.startDate && period.endDate ? (
                    ...
                    <span className="text-gray-500 italic">Please select start and end dates</span>
                  )}
                </div>
              </div>
            </div>
