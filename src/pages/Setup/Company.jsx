  ...
  useEffect(() => {
    if (editSuccess && lastAction) {
      ...
      if (lastAction.type === 'toggle') {
        toast.success(`Company ${lastAction.status ? 'activated' : 'deactivated'} successfully`, {
          iconTheme: {
  ...
  const allColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          const date = getValue();
          return date ? <div className="flex justify-center">{formatToDDMMYYYY(date)}</div> : null;
        },
      },
      {
        accessorKey: 'status',
