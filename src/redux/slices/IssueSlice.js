...
export const fetchIssue = createAsyncThunk(
  'fetchIssue',
  async ({ token, page = 1, per_page = 10 }) => {
    try {
...
export const attachFile = createAsyncThunk('updateIssue', async ({ token, id, payload }) => {
  try {
    const response = await axios.put(`${baseURL}/issues/${id}.json`, payload, {
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'Multipart/form-data',
      },
...
export const filterIssue = createAsyncThunk(
  'filterIssue',
  async ({ token, filter }, { rejectWithValue }) => {
    try {
      const params = new URLSearchParams(filter).toString();
      console.log(params);
      const response = await axios.get(`${baseURL}/issues.json?${params}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const removeIssueAttachment = createAsyncThunk(
  'removeIssueAttachment',
  async ({ token, id, image_id }) => {
    try {
      const response = await axios.delete(
        `${baseURL}/issues/${id}/remove_attachemnts/${image_id}.json`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
...
export const removeIssueAttachmentSlice = createApiSlice(
  'removeIssueAttachment',
  removeIssueAttachment
);
